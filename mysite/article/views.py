...
from.forms import AtricleColumnForm,ArticlePostForm,ArticleTagForm
...
@csrf_exempt             #解决提交表单的 csrf 问题
def article_column(request):
    if request.method == "GET":
        columns = ArticleColumn.objects.filter(user=request.user)
        column_form =AtricleColumnForm()
...
def rename_article_column(request):
    ...
    try:
        line = ArticleColumn.objects.get(id=column_id)    #查询该数据
        line.column = column_name                         #修改数据的column
        line.save()
        return HttpResponse("1")
    except:
    ...
    if request.method == "POST":
        ...
        if article_post_form.is_valid():
            ...
            try:
                ...
                new_article.save()
                tags = request.POST['tags']  # 获取前端传过来的标签
                if tags:
                    # JSON 数据转换为列表
                    for atag in json.loads(tags):
                        tag = request.user.tag.get(tag=atag)
                        new_article.article_tag.add(tag)     # 实现文章和文章标签之间的对应关系
...
def article_list(request):
    ...
    page = request.GET.get('page')          # 获取 URL中的page 参数
    try:
...
def article_detail(request,id,slug):
    article=get_object_or_404(ArticlePost,id=id,slug=slug)       #查询 对应 id ,slug 的对象
    return render(request,"article/column/article_detail.html",{"article": article})    #硬编码渲染页面
...
def redit_article(request, article_id):
    if request.method == "GET":
        article_columns = request.user.article_column.all()         #反查询栏目
        article = ArticlePost.objects.get(id=article_id)            #查询改ID 的文章
...
def article_tag(request):
    if request.method == "GET":
        article_tags = ArticleTag.objects.filter(author=request.user)    #  筛选是当前用户的标签
    ...
    if request.method == "POST":
        ...
        if tag_post_form.is_valid():
            ...
            return HttpResponse("抱歉，数据不是有效的")
